# Meraki MSP Toolkit v0.2.0 - Final Regression Checklist

This is a short smoke/regression pass for the final package. Do not add new features during this pass.

1. Launch with `Launch-Meraki-MSP-Toolkit.bat`; confirm v0.2.0 and no persistent console window.
...
4. Hardware / Inventory dry run; verify exact source org, target org/network, serial, and stored site address.
...
Release acceptance: no unexpected write, no raw JSON in user-facing Post-Build reports, no `ERAKI` headings, no stale RC version strings in executable/launcher UI, and no ZIP integrity errors.
